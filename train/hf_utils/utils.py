...
#code /envs/beamS/lib/python3.10/site-packages/transformers/generation/utils.py
import copy
import inspect
import os
import warnings
...
class GenerationMixin(ContinuousMixin):
    ...
    def _get_top_k_continuations(
        ...
        decoder_prompt_len: int,
        do_sample: bool,
        topk_per_beam: int, #,beams_to_keep: int,   
        num_beams: int,
        vocab_size: int,
        batch_size: int,
        use_likelihood=None,
        ...
        if do_sample:
            topk_indices = torch.multinomial(
                nn.functional.softmax(accumulated_log_probs, dim=-1), num_samples=topk_per_beam
            )
            topk_log_probs = torch.gather(input=accumulated_log_probs, dim=1, index=topk_indices)
        else:
            topk_log_probs, topk_indices = torch.topk(accumulated_log_probs, k=topk_per_beam)

        # Gather K top beams, recover the beam index by floor division and token id by modulo division
        topk_likelihood=None
        if use_likelihood:
            topk_likelihood = torch.gather(likelihood, dim=1, index=topk_indices)
        ...
        return topk_log_probs, topk_running_sequences, topk_running_beam_indices,topk_likelihood
    ...
    def _get_running_beams_for_next_iteration(
        ...
        if use_reward:
            return running_sequences, running_beam_scores, running_beam_indices,next_topk_indices
        return running_sequences, running_beam_scores, running_beam_indices, None
    def _update_finished_beams(
        ...
        #print("did_top_num_beams_just_finished",did_top_num_beams_just_finished.shape  ,did_top_num_beams_just_finished)
        if not use_reward:
       ...
       # print("beams_in_batch_are_full",beams_in_batch_are_full)
        topk_log_probs += beams_in_batch_are_full.to(torch.float32) * -1.0e9
       # print("+= beams_in_batch_are topk_log_probs",topk_log_probs)
       ...
       # print("merged_beam_indices",merged_beam_indices)
        merged_is_sent_finished = torch.cat((is_sent_finished, did_top_num_beams_just_finished), dim=1)
       # print("merged_is_sent_finished",merged_is_sent_finished)
       ...
       # print("alll --------",sequences, beam_scores, beam_indices, is_sent_finished)
       # print("************************************************")
        return sequences, beam_scores, beam_indices, is_sent_finished

    # end of auxiliary functions for beam search
    def _get_top_k_continuations_per_beam(
    self,
    accumulated_log_probs: torch.Tensor,         # [B, nb, V]
    running_sequences: torch.Tensor,       # [B, nb, T]
    running_beam_indices: torch.Tensor,    # [B, nb, T’]
    cur_len: int,
    decoder_prompt_len: int,
    do_sample: bool,
    topk_per_beam: int, #,beams_to_keep: int,                   # = 5
    num_beams: int,
    vocab_size: int,
    batch_size: int,
    use_likelihood=None,
    likelihood=None,
        ...
        #step_log_probs=accumulated_log_probs
        #topk_per_beam=beams_to_keep
        topk_per_beam=int(topk_per_beam/num_beams)
        B, nb, V = accumulated_log_probs.shape
        assert nb == num_beams and V == vocab_size
        if do_sample:
            # sample k tokens per beam without replacement
            probs = torch.softmax(accumulated_log_probs, dim=-1)                 # [B, nb, V]
            flat = probs.view(B * nb, V)
            ids = torch.multinomial(flat, num_samples=topk_per_beam)      # [B*nb, k]
            topk_ids = ids.view(B, nb, topk_per_beam)
            topk_log_probs = torch.gather(accumulated_log_probs, -1, topk_ids)   # [B, nb, k]
        else:

            topk_log_probs, topk_ids = torch.topk(accumulated_log_probs, k=topk_per_beam, dim=-1)  # [B, nb, k]

        beams_to_keep = num_beams * topk_per_beam  # 25
        if use_likelihood :
        # [B, nb, k] likelihood of the chosen tokens in topk_ids
            topk_log_likelihood = torch.gather(likelihood, -1, topk_ids)
            # flatten exactly like ids/logprobs: [B, beams_to_keep]
            cand_log_likelihood = topk_log_likelihood.reshape(B, beams_to_keep)

        # flatten candidates: [B, 25]
        cand_log_probs = topk_log_probs.reshape(B, beams_to_keep)
        cand_ids = topk_ids.reshape(B, beams_to_keep)

        # source beam index for each candidate
        src_beam = (
            torch.arange(num_beams, device=accumulated_log_probs.device)
            .view(1, num_beams, 1)
            .expand(B, num_beams, topk_per_beam)
            .reshape(B, beams_to_keep)
        )

        cand_running_beam_indices = self._gather_beams(running_beam_indices, src_beam)
        cand_running_sequences = self._gather_beams(running_sequences, src_beam)

        # append selected token
        cand_running_sequences[:, :, cur_len] = cand_ids

        batch_offset = torch.arange(batch_size, device=cand_ids.device).view(-1, 1) * num_beams
        batch_modified_indices = src_beam + batch_offset
        cand_running_beam_indices[:, :, cur_len - decoder_prompt_len] = batch_modified_indices
        if use_likelihood :
            #print("ite jussed use_likelihood")
            return cand_log_probs, cand_running_sequences, cand_running_beam_indices,cand_log_likelihood
        return cand_log_probs, cand_running_sequences, cand_running_beam_indices,None
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        Return:
            ...
            `model.config.is_encoder_decoder=True`.
        """
        tokenizer = getattr(self, "_bm25_tokenizer", None)
        reward_fn = getattr(self, "_bm25_reward_fn", None)
        reward_k  = int(getattr(self, "_bm25_reward_k", 100))
        qids      = getattr(self, "_bm25_qids", None)
        self.epsilon_base =  getattr(self, "_epsilon_base", 1e-4)
        use_reward = reward_fn is not None
        #print("use_reward",use_reward,reward_fn)
        topk_per_beam = int(getattr(self, "_bm25_topk_per_beam", 2))
        show_more = (getattr(self, "_show_more", False))
        use_likelihood= (getattr(self, "_use_likelihood", False))
        reward_cfg=  str(getattr(self, "_reward_cfg", "hard"))
        #print("We are using likelihood,",use_likelihood)
        likelihood=None
        ...
        beams_to_keep = max(topk_per_beam, 1 + n_eos_tokens) * num_beams
        if  use_reward:
            top_num_beam_mask = torch.ones(beams_to_keep, dtype=torch.bool, device=input_ids.device)
        else:
        ...
        if use_likelihood: 
            running_beam_probos = torch.zeros((batch_size, num_beams), dtype=torch.float, device=input_ids.device)
            running_beam_probos[:, 1:] = -1e9
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if return_dict_in_generate:
                print("return_dict_in_generate is activated ")
                if output_logits:
            ...
            if use_likelihood:
                #print("shapes",log_probs.shape,running_beam_probos.shape,running_beam_probos[:, :, None].shape)
                likelihood = log_probs + running_beam_probos[:, :, None]
            ## i need remove running_beam_scores
            if not use_reward or decoder_prompt_len==cur_len :
                log_probs = log_probs + running_beam_scores[:, :, None]
            #should stop reshape here 
                log_probs = torch.reshape(log_probs, (batch_size, num_beams * vocab_size))
                if use_likelihood: likelihood  = torch.reshape(likelihood, (batch_size, num_beams * vocab_size))
                _get_top_k_continuations=self._get_top_k_continuations
            else:
                _get_top_k_continuations=self._get_top_k_continuations_per_beam
            ...
            #I have to use topk with dim 3

            topk_log_probs, topk_running_sequences, topk_running_beam_indices,cand_log_likelihood = _get_top_k_continuations( #_get_top_k_continuations(
            ...
            #print("we are inside gene")
            if use_reward:
                if show_more: print("we are inside reward_fn")
                cand_tokens = topk_running_sequences[:, :, decoder_prompt_len:cur_len+1] 
                cand_texts = tokenizer.batch_decode(
                    cand_tokens.reshape(-1, cand_tokens.size(-1)),
                    skip_special_tokens=True,
                    )
                ndcg_scores = []
                for b in range(batch_size):
                    texts_b = cand_texts[b*beams_to_keep:(b+1)*beams_to_keep]
                    
                    
                    s = reward_fn(texts_b, qid=qids[b], k=reward_k,log_likelihood=cand_log_likelihood[b],reward_cfg=reward_cfg)  # [25]
                    #indices = torch.arange(s.shape[-1], device=s.device)
                    #cyclic_multiplier = (topk_per_beam - (indices % topk_per_beam))* self.epsilon_base
                    #s = s + cyclic_multiplier
                ...
                ndcg_scores = torch.stack(ndcg_scores, dim=0).to(topk_running_sequences.device)
                
                del cand_tokens , cand_texts#, indices,cyclic_multiplier
                # Your reward/score fn: text -> float

                # d. Check which running sequences have finished
            else:ndcg_scores = topk_log_probs
            ...
            # i need to put ndcg@10
            running_sequences, running_beam_scores, running_beam_indices,next_topk_indices = self._get_running_beams_for_next_iteration(
                topk_log_probs=ndcg_scores,#topk_log_probs,
                ...
                num_beams=num_beams,use_reward=use_likelihood
            )
            if use_likelihood:
                running_beam_probos= self._gather_beams(cand_log_likelihood, next_topk_indices)
                del next_topk_indices,cand_log_likelihood
            ...
            #print("english sequences",tokenizer.batch_decode(sequences.squeeze(0),skip_special_tokens=True))
            ...
            cur_len = cur_len + 1
            if show_more:print("We are at step",cur_len)
